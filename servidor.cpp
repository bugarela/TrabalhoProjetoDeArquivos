...
dado criaDado(string linhaRegistros){
	...
	try {
		if(linhaRegistros.find(';') > linhaRegistros.size()){
			...
			int primeiro = linhaRegistros.find(';');
			busca.RowID = stoi(linhaRegistros.substr(0,primeiro));
			busca.OrderID = stoi(linhaRegistros.substr(primeiro+1,linhaRegistros.find(';',primeiro+1)));
			busca.tamanho = linhaRegistros.size();
		}
	} catch (exception& ex) {
		cout << ">stoi exception: " << busca.OrderID << " " << busca.RowID << endl;
	}
...
map <dado,int> insereIndice (string linhaRegistros, map <dado,int> arvore){
	...
	cout << ">" << novoDado.OrderID << " inserido" << endl;
				...
				while(1){
					...
					switch(comando[0]){
						...
						case 's':
							...
							atualizaArquivos(arvore);
							new_sock << "Arquivos salvos";
							break;
					}
